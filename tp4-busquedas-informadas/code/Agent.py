...
class Agent:
    ...
    def h(self,state):
        x = self.env.goalPos[0] - state[0]
        y = self.env.goalPos[1] - state[1]
    ...
    def __think(self,treeNode,frontier,exploredNodes,Tree,cost,limit,isAStar): #implementa las acciones a seguir
        """ 
                ...
                if self.__is_inside(newState) == True:
                    if self.__is_obstacle(newState) == False:
                        if self.__already_explored(newState,exploredNodes) == False:
                            ...
                            if isAStar == True:
                                #A*
                                newcost = self.g(cost) + self.h(newState)
                            else:
                                #UniformCost
                                newcost = self.g(cost)
                            if isinstance(frontier,myQueue.priorityQueue) and frontierNode != None:
                                    #code for Uniform cost Search y A*
                            ...
                            if (lessCost and isinstance(frontier,myQueue.priorityQueue)) or frontierNode == None:
                                ...
                                frontier.add(Tree.get_node(str(newState)),newcost) # costo de Uniform Cost Search
    ...
    def __exploringFrontier(self,Tree,frontier,explored,limit,isAStar):
        while frontier.size>0:
            ...
            if self.env.ground[treeNode.data[0]][treeNode.data[1]] == "><":
                ...
                cost = node.priority
                self.__think(treeNode,frontier,explored,Tree,cost,limit,isAStar)
    ...
    def __getSolution(self,frontier,limit,isAStar):
        T = self.__startTreeAndFrontier(frontier)
        explored = myQueue.PythonList()
        solution = self.__exploringFrontier(T,frontier,explored,limit,isAStar)
        del frontier
        del explored
        del T
        return solution
    ...
    def solveByBFS(self):
        ...
        return self.__getSolution(frontier,limit,False)
    ...
    def solveByAStar(self):
        ...
        frontier = myQueue.priorityQueue()
        limit= self.env.sizes[0]*self.env.sizes[1] # el limite mas alto posible
        return self.__getSolution(frontier,limit,True)
